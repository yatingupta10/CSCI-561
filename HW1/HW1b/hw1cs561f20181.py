...
def delete_elements(sorted_nodes_set, ref_element):   ####ref_element = ((0,0), 5)
	delete_elements_list = list()
	for j in range(len(sorted_nodes_set)):
		for k in range(j + 1, len(sorted_nodes_set)):
			if sorted_nodes_set[j][0][0] == ref_element[0][0]:
				delete_elements_list.append(sorted_nodes_set[k])
			
			elif sorted_nodes_set[j][0][1] == ref_element[0][1]:
				delete_elements_list.append(sorted_nodes_set[k])

			elif sorted_nodes_set[j][0][0] - ref_element[0][1] == sorted_nodes_set[k][0][0] - ref_element[0][1]:
				delete_elements_list.append(sorted_nodes_set[k])

			elif sorted_nodes_set[j][0][0] + ref_element[0][1] == sorted_nodes_set[k][0][0] + ref_element[0][1]:
				delete_elements_list.append(sorted_nodes_set[k])

	return delete_elements_list
...
sorted_nodes_set = sorted(nodes_dict.items(), key=lambda x: x[0])
# print sorted_nodes_set

print delete_elements(sorted_nodes_set, ((0,0), 0))
